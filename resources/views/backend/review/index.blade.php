          ...
          <tbody>
            @foreach($reviews as $review)
                <tr>
                    ...
                    @php
                        $time = $review->created_at->setTimezone('Asia/Jakarta');
                        $hour = $time->format('H');
                        $timeOfDay = ($hour < 12) ? 'pagi' : (($hour < 15) ? 'siang' : (($hour < 19) ? 'sore' : 'malam'));
                    @endphp
                    <td>{{ $time->translatedFormat('d F Y, H:i') }} WIB ({{ $timeOfDay }})</td>
