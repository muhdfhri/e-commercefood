...
class FrontendController extends Controller
{
    ...
    public function productDetail($slug){
        $product_detail = Product::with(['cat_info', 'sub_cat_info', 'brand'])
            ->where('slug', $slug)
            ->where('status', 'active')
            ->first();
        ...
        $variants = [];
        if(!empty($product_detail->variant)) {
            $variants = explode(',', $product_detail->variant);
            $variants = array_map('trim', $variants);
            $variants = array_filter($variants); // Hapus yang kosong
        }
    ...
    public function productCat(Request $request, $slug = null) {
        try {
            // Pastikan slug tersedia
            $slug = $slug ?? $request->input('slug');
            
            // Handle request untuk service worker
            if ($slug === 'sw.js') {
            ...
            if (empty($slug)) {
                \Log::warning('Category slug is empty');
                abort(404, 'Kategori tidak ditemukan');
            }

            // Dapatkan kategori beserta produk aktif
            $category = Category::with(['products' => function($query) {
                $query->where('status', 'active');
            }])->where('slug', $slug)->first();

            if (!$category) {
                \Log::warning('Category not found for slug: ' . $slug);
                abort(404, 'Kategori tidak ditemukan');
            }
            ...
            \Log::info("Found {$products->total()} products for category: {$category->title}");

            // Tentukan view yang akan digunakan
            $view = str_contains(request()->path(), 'product-grids') 
            ...
            \Log::error('Error in productCat: ' . $e->getMessage() . '\n' . $e->getTraceAsString());
            return redirect()->route('home')
                           ->with('error', 'Maaf, terjadi kesalahan saat memuat produk. Silakan coba lagi nanti.');
        }
    }
    ...
    public function registerSubmit(Request $request){
        ...
        if($check){
            ...
            request()->session()->flash('error','Silahkan coba lagi!');
            return back();
        }
    }
    ...
    public function subscribe(Request $request){
        if(! Newsletter::isSubscribed($request->email)){
                ...
                if(Newsletter::lastActionSucceeded()){
                    ...
                    return back()->with('error','Gagal subscribe! Silahkan coba lagi');
                }
            }
            else{
                request()->session()->flash('error','Sudah subscribe!');
                return back();
            }
    }
